...
def preprocess_text(abstract):
    ...
    abstract = re.sub(r'\([a-zA-Z]+ et al\., \d{4}\)', '', abstract)
...
def preProcessText(text, isalpha=False, stopwords=False):
    ...
    tokens = [word for word in tokens if word.isalpha()] if isalpha else tokens
    # Remove stopwords
    tokens = [
        word for word in tokens if word not in stop_words] if stopwords else tokens
    return " ".join(tokens)
...
def splitXY(df):
    ...
    x = pd.DataFrame(
        {"text": df["titles"].values + " " + df["summaries"].values})["text"].values
...
def makeDateloader(XY_sets):
    ...
    for (x_vec, y_vec, shuffle) in XY_sets:
        dataset = TensorDataset(torch.FloatTensor(
            x_vec), torch.FloatTensor(y_vec))
        dataloader = DataLoader(
            dataset, batch_size, pin_memory=True, shuffle=shuffle, worker_init_fn=seed_worker)
...
def prepareData(file_path, reduce_labels_rate=0):
    ...
    vectorizer = CountVectorizer(
        analyzer='word', ngram_range=(1, 2), max_features=99999)
